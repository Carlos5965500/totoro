  ...
  async execute(totoro, msg, args) {
    ...
    const sendErrorMessage = async () => {
      await sendError(
        totoro,
        msg,
        `${isUrl ? "No se pudo descargar la canción." : "No se encontraron resultados para la búsqueda."}`
      );
    };
    ...
    const handleDownload = async (song) => {
      try {
        ...
        const songDetails =
          ...
          `│  ≡◦ * ⏱ *Duración:* ${(song.duration / 60000).toFixed(2)} minutos \n` +
          `│  ≡◦ * ▶ *Reproducciones:* ${song.playCount} \n` +
          `│  ≡◦ * 💬 *Comentarios:* ${song.commentsCount} \n` +
          `│  ≡◦ * ❤ *Likes:* ${song.likes}  \n` +
          `│  ≡◦ * 🎶 *Género:* ${song.genre} \n ` +
          `│  ≡◦ * 📅 *Publicado en:* ${new Date(song.publishedAt).toLocaleDateString()} \n` +
          `│  ≡◦ * 🔗 *URL:* ${song.url} \n` +
          `╰─⬣\n\n` +
          `> Solicitado por: @${participant.split("@")[0]}`;
